...
@testset "Zernike radial function" begin
    @test ShackHartmann.R(0.5; n=1, m=1) isa Real
...
@testset "Index switching" begin
    @test double2single_index(0, 0) == 0
    @test double2single_index(1, -1) == 1
    @test double2single_index(2, 0) == 4

    @test single2double_index(0) == (0,0)
    @test single2double_index(1) == (1,-1)
    @test single2double_index(4) == (2,0)
end